...
LINEAR_WAVE_DIRECTIONS_HYDRO_OPTIONS = hydro_cli_options(LINEAR_WAVE_DIRECTIONS)
LINEAR_WAVE_DIRECTIONS_OUTPUT_VARIABLES = output_variable_list(
    LINEAR_WAVE_DIRECTIONS
)
...
rule run_linear_wave_direction:
    ...
    shell:
        ...
        {input.executable} -i {input.deck} \
          ...
          {LINEAR_WAVE_DIRECTIONS_HYDRO_OPTIONS} \
          hydro/gamma=1.666666666666667 \
          parthenon/output0/file_type=hdf5 \
          parthenon/output0/dt=-0.01 \
          parthenon/output0/variables={LINEAR_WAVE_DIRECTIONS_OUTPUT_VARIABLES} \
          > {log.out} 2> {log.err}
        """
