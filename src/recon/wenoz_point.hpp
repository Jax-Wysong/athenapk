...
//========================================================================================
#ifndef RECONSTRUCT_WENOZ_POINT_HPP_
#define RECONSTRUCT_WENOZ_POINT_HPP_
//! \file wenoz_point.hpp
//  \brief WENO-Z point-wise reconstruction for a Cartesian-like coordinate with uniform spacing.
//
// REFERENCES:
// Berta et al. (2024) A 4th-order accurate finite volume method 
// for ideal classical and special relativistic MHD based on pointwise reconstructions
...
void WENOZ_POINT(const Real &q_im2, const Real &q_im1, const Real &q_i, const Real &q_ip1,
           const Real &q_ip2, Real &ql_ip1, Real &qr_i) {
            
  const Real beta_coeff[2]{13. / 12., 0.25};

  // (jwysong) beta remains unchanged
  Real beta[3];
  ...
  f[1] = (-1.0 * q_im1 + 6.0 * q_i + 3.0 * q_ip1);
  ...
  // d1:   0.6   -->     5/8
  // d2:   0.3   -->     5/16
  alpha[0] = (1.0/16.0) * (1.0 + indicator[0]);
  alpha[1] = (5.0/8.0)  * (1.0 + indicator[1]);
  alpha[2] = (5.0/16.0) * (1.0 + indicator[2]);
  // 8.0 instead of 6.0 
  ...
  f[1] = (-1.0 * q_ip1 + 6.0 * q_i + 3.0 * q_im1);
  f[2] = (3.0 * q_i + 6.0 * q_im1 - q_im2);

  alpha[0] = (1.0/16.0) * (1.0 + indicator[2]);
  alpha[1] = (5.0/8.0)  * (1.0 + indicator[1]);
  alpha[2] = (5.0/16.0) * (1.0 + indicator[0]);
  alpha_sum = 8.0 * (alpha[0] + alpha[1] + alpha[2]);

  qr_i = (f[0] * alpha[0] + f[1] * alpha[1] + f[2] * alpha[2]) / alpha_sum;
}

//! \fn Reconstruct<Reconstruction::wenoz_point, int DIR>()
//  \brief Wrapper function for WENOZ_POINT reconstruction
...
KOKKOS_INLINE_FUNCTION typename std::enable_if<recon == Reconstruction::wenoz_point, void>::type
  ...
  for (auto n = 0; n < nvar; ++n) {
    parthenon::par_for_inner(member, il, iu, [&](const int i) {
      if constexpr (XNDIR == parthenon::X1DIR) {
        ...
        WENOZ_POINT(q(n, k, j, i - 2), q(n, k, j, i - 1), q(n, k, j, i), q(n, k, j, i + 1),
              q(n, k, j, i + 2), ql(n, i + 1), qr(n, i));
      } else if constexpr (XNDIR == parthenon::X2DIR) {
        // ql is ql_jp1 and qr is qr_j
        WENOZ_POINT(q(n, k, j - 2, i), q(n, k, j - 1, i), q(n, k, j, i), q(n, k, j + 1, i),
              q(n, k, j + 2, i), ql(n, i), qr(n, i));
      } else if constexpr (XNDIR == parthenon::X3DIR) {
        // ql is ql_kp1 and qr is qr_k
        WENOZ_POINT(q(n, k - 2, j, i), q(n, k - 1, j, i), q(n, k, j, i), q(n, k + 1, j, i),
              q(n, k + 2, j, i), ql(n, i), qr(n, i));
      } else {
        PARTHENON_FAIL("Unknow direction for wenoz_point reconstruction.")
      }
    });
  }
}

#endif // RECONSTRUCT_WENOZ_POINT_HPP_
