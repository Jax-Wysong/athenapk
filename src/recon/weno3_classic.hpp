...
//========================================================================================
#ifndef RECONSTRUCT_WENO3_CLASSIC_HPP_
#define RECONSTRUCT_WENO3_CLASSIC_HPP_
//! \file weno3_classic.hpp
...
void WENO3JS(const Real &q_im1, const Real &q_i, const Real &q_ip1, Real &ql_ip1, Real &qr_i) {
  ...
  ql_ip1 = (alpha[0] * f[0] + alpha[1] * f[1]) / alpha_sum; // (2.52) 
  ...
  qr_i = (alpha[0] * f[0] + alpha[1] * f[1]) / alpha_sum; // (2.52) 
...
KOKKOS_INLINE_FUNCTION typename std::enable_if<recon == Reconstruction::weno3js, void>::type
  ...
  for (auto n = 0; n < nvar; ++n) {
    parthenon::par_for_inner(member, il, iu, [&](const int i) {
      ...
      if constexpr (XNDIR == parthenon::X1DIR) {
        ...
        WENO3JS(q(n, k, j, i - 1), q(n, k, j, i), q(n, k, j, i + 1), ql(n, i + 1), qr(n, i));
      } else if constexpr (XNDIR == parthenon::X2DIR) {
        // ql is ql_jp1 and qr is qr_j
        WENO3JS(q(n, k, j - 1, i), q(n, k, j, i), q(n, k, j + 1, i), ql(n, i), qr(n, i));
      } else if constexpr (XNDIR == parthenon::X3DIR) {
        // ql is ql_kp1 and qr is qr_k
        WENO3JS(q(n, k - 1, j, i), q(n, k, j, i), q(n, k + 1, j, i), ql(n, i), qr(n, i));
      } else {
...
#endif // RECONSTRUCT_WENO3_CLASSIC_HPP_